    ...
    <div class="flex flex-col">
        <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
            ...
            $search=isset($_GET['name'])? $_GET['name']: "";
				if ($search){
					$sqls="SELECT * FROM bookroom WHERE roomname LIKE '%".$search."%'";  
					$result=mysqli_query($conn,$sqls);    
                    if(mysqli_num_rows($result)>0){                           
                        echo"<table class = 'min-w-full divide-y divide-gray-200'>";      
                        echo"<thead id='table-detail' class='bg-gray-50'>" ; 
                        echo "<tr style='text-align: center'>
                        <th scope='col' class='' style='width :5%'>ID</th>
                        <th scope='col' class='' style='width :10%'>Room Name</th>
                        <th scope='col' class='' style='width :10%'>Guest Name</th>
                        <th scope='col' class='' style='width :15%'>Email</th>
                        <th scope='col' class='' style='width :10%'>Code</th>
                        ...
                        <th scope='col' class='' style='width :5%'>Guests</th>
                        <th scope='col' class='' style='width :5%'>Rooms</th>
                        ...
                        while($row=mysqli_fetch_assoc($result)){
                            echo "<tr>";        
                                    echo "<td class='text-center'>".$row["idbook"]."</td>";                  
                                    echo "<td class='text-center'>".$row["roonname"]."</td>";                  
                                    ...
                                    echo "<td class='text-center'><a class='text-dark border rounded  bg-info' href=\"checkroom.php?BookID=".$row['idbook']."\"><i  class=fa fa-check text-danger text-center' style='margin:15px;width:50px'>OK</i></td>";  
                                    echo "<td class='text-center'><a class='text-dark border rounded bg-danger' href=\"includes/deletebookroom.php?BookID=".$row['idbook']."\"><i  class='fa fa-trash text-white text-center' style='margin:15px; width:50px'> Xóa</i></a></td>";
                            echo "</tr>";
                        }
                        echo "</table>";
                    }    
                ...
                $result = mysqli_query($conn, 'select * from bookroom');
                    ...
                    $result = mysqli_query($conn, "SELECT * FROM bookroom LIMIT $start, $limit");  
